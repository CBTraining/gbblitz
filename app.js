...
class StarrySkyBackground {
  ...
  render(now) {
    ...
    // 3. Faint interactive constellation threads connecting mouse to nearby stars
    if (this.mouse.hover) {
      this.ctx.beginPath();
      let connectCount = 0;
    ...
    // 5. 3D Stars: mostly crisp white with speckled Google colors
    for (let i = 0; i < this.stars.length; i++) {
      const s = this.stars[i];
